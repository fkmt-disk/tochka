...
object ReflectionUtil {
  ...
  implicit
  class InstanceMirrorW(instance: InstanceMirror) {
  ...
  def isCaseClass(typ: Type): Boolean = isCaseClass(typ.toClass)
  
  def isCaseClass(any: Any): Boolean = Option(any).map(x => isCaseClass(x.getClass)).getOrElse(false)
